...
describe("export compatibility", () => {
  ...
  it("round-trips every operational field", () => {
    const data = validApp();
    data.tripsByVehicle.v1[0].legs[0].driver = "Retro"; data.tripsByVehicle.v1[0].legs[0].passengers = ["Ambassador", "Security Officer"];
  ...
  it("preserves leg people in report JSON", () => { const trip = { id: "t", legs: [{ id: "l", driver: "Retro", passengers: ["Ambassador"] }] }; expect(createReportExport({ range: {}, vehicle: {}, trips: [trip], fuel: [] }).trips[0]).toEqual(trip); });
});
